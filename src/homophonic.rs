...
//!
use crate::common::alphabet::{Alphabet, ALPHABET_LOWER};
...
impl Cipher for Homophonic {
    ...
    ///
    /// # Panics
    /// * The `key` does not contain a mapping for .
    /// * The `key` is empty.
    ///
    fn new(key: HashMap<char, Vec<char>>) -> Homophonic {
        //
        if !ALPHABET_LOWER.iter().all(|c| key.contains_key(c)) {
            panic!("The `key` does not contain a mapping for all characters in the lower case alphabet.");
        }
